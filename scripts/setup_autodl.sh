...
# 先检查是否已安装
if python -c "import flash_attn; print(flash_attn.__version__)" 2>/dev/null | grep -q "$FLASH_ATTN_VERSION"; then
    echo "  Flash Attention $FLASH_ATTN_VERSION 已安装，跳过"
else
    ...
    # 先尝试直接下载预编译 wheel（--max-time 防止 GitHub 连不上时无限挂起）
    if curl -L --connect-timeout 15 --max-time 120 -o "/tmp/${FLASH_ATTN_WHEEL}" "$FLASH_ATTN_URL" 2>/dev/null && [ -s "/tmp/${FLASH_ATTN_WHEEL}" ]; then
